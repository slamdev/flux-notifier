build:
	go build -o bin/flux-notifier ./cmd/notifier

run: build
	./bin/flux-notifier listen
...
untag:
	...
	curl --request DELETE --header "Authorization: token ${GITHUB_TOKEN}" "https://api.github.com/repos/slamdev/flux-notifier/releases/:release_id/$(TAG)"
